...
void* bake_orders(void* arg) {
    ...
    while (true) {
        ...
        while (bread_count > 0) {
            ...
            pthread_mutex_lock(&print_mutex);
            cout << "Baker " << baker_id + 1 << " cooked " << bake_now << " breads for " << name << "\n";
            pthread_mutex_unlock(&print_mutex);
...
void simulate_bakery(vector<vector<Order>> &orders, int num_bakers) {
    total_oven_capacity = num_bakers * MAX_BREADS;
    ...
    for (size_t i = 1; i < completed_orders.size(); ++i) {
        if (completed_orders[i].order_index < completed_orders[i - 1].order_index) {
            disorder_count++;
        }
    }

    double disorder_percentage = (double(disorder_count) / completed_orders.size()) * 100;
    cout << "Disorder count: " << disorder_count << " orders (" << disorder_percentage << "%).\n";
}
    ...
    for (int i = 0; i < num_bakers; ++i) {
        ...
        for (size_t j = 0; j < names.size(); ++j) {
            ...
            if (order > MAX_ORDER) {
                ...
                orders[i].push_back({names[j], order, order_index++});
            }
        }
    }
...
int main() {
    ...
    available_spaces.resize(num_bakers, MAX_BREADS);
